...
static void update_square(int16_t* samples, const bool ch2, uint_fast16_t sample_words)
{
    uint32_t freq;
    ...
    for (uint_fast16_t i = 0; i < sample_words; i += 2) {
        update_len(c);
...
static void update_wave(int16_t *samples, uint_fast16_t sample_words)
{
    uint32_t freq;
...
static void update_noise(int16_t *samples, uint_fast16_t sample_words)
{
...
void audio_callback(void *userdata, uint8_t *stream, int len)
{
    ...
    uint_fast16_t sample_words = (uint_fast16_t)(len / (int)sizeof(int16_t));
    sample_words &= ~1u;
    if (sample_words == 0) {
        memset(stream, 0, len);
        return;
    ...
    update_square(samples, 0, sample_words);
    update_square(samples, 1, sample_words);
    update_wave(samples, sample_words);
    update_noise(samples, sample_words);
}
