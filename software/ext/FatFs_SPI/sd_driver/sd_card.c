...
static inline void sd_debug_log(const char *msg) {
    if (msg == NULL) {
        return;
    }
...
static int sd_init_medium(sd_card_t *pSD) {
    ...
    */
    ...
    sd_debug_log("Init: SPI clock throttled to 400kHz");
    sd_debug_log("Init: sending 80 dummy clocks");
    sd_spi_send_initializing_sequence(pSD);
    sd_debug_log("Init: dummy clocks complete");

    // The card is transitioned from SDCard mode to SPI mode by sending the CMD0
    // + CS Asserted("0")
    sd_debug_log("Init: requesting idle state via CMD0");
    if (sd_go_idle_state(pSD) != R1_IDLE_STATE) {
        sd_debug_log("Init: CMD0 failed to enter idle state");
        DBG_PRINTF("No disk, or could not put SD card in to SPI idle state\r\n");
        return SD_BLOCK_DEVICE_ERROR_NO_DEVICE;
    }
    sd_debug_log("Init: card entered idle state");
    ...
    sd_debug_log("Init: probing card with CMD8");
    status = sd_cmd8(pSD);
    if (SD_BLOCK_DEVICE_ERROR_NONE != status &&
        SD_BLOCK_DEVICE_ERROR_UNSUPPORTED != status) {
        sd_debug_log("Init: CMD8 probe failed");
        return status;
    }
    if (SD_BLOCK_DEVICE_ERROR_NONE == status) {
        sd_debug_log("Init: CMD8 indicates v2 card");
    } else {
        sd_debug_log("Init: CMD8 unsupported; assuming legacy card");
    }
    ...
    sd_debug_log("Init: reading OCR via CMD58");
    if (SD_BLOCK_DEVICE_ERROR_NONE !=
        (status = sd_cmd(pSD, CMD58_READ_OCR, 0x0, false, &response))) {
        sd_debug_log("Init: CMD58 failed");
        return status;
    }
     ...
     */
    sd_debug_log("Init: waiting for ACMD41 ready state");
    ...
    if ((SD_BLOCK_DEVICE_ERROR_NONE != status) || (0x00 != response)) {
        ...
        sd_debug_log("Init: ACMD41 timed out");
        return status;
    }
    sd_debug_log("Init: ACMD41 complete");
    ...
    if (SDCARD_V2 == pSD->card_type) {
        ...
        if (SD_BLOCK_DEVICE_ERROR_NONE ==
            ...
            if (response & OCR_HCS_CCS) {
                ...
                sd_debug_log("Init: detected v2 standard capacity card");
            }
        }
    } else {
        pSD->card_type = SDCARD_V1;
        DBG_PRINTF("Card Initialized: Version 1.x Card\r\n");
        sd_debug_log("Init: detected v1 card");
    }
...
bool sd_init_driver() {
    ...
    if (!initialized) {
        ...
        for (size_t i = 0; i < spi_get_num(); ++i) {
            ...
            //TESTING!!! EARLY EXIT HERE PREVENTS LOCKUP
